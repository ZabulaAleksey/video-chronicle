...
и собирает в один MP4-файл. Chronicle может добавлять на кадр дату и время, а Join
...
- В Chronicle добавляет настраиваемые дату/время; Join отключает подпись.
...
В окне можно выбрать Chronicle/Join, входную папку, итоговый MP4 и параметры
даты/времени Chronicle. Вкладка «Дата и время» открывается первой. Автоматически
разрешённые пути FFmpeg/FFprobe, CRF, preset и cache доступны во вторичной
вкладке «Дополнительно». Перед экспортом GUI
показывает состав/order плана и representative кадр.
После анализа можно сохранить/открыть project JSON, изменить порядок элементов,
