...
- representative-frame preview имеет отдельные stale/loading/ready/disabled/
  error состояния; экспорт доступен только для актуального preview;
...
GUI пока не поддерживает ручной reorder/trim, воспроизведение preview, проценты,
безопасную отмену и сохранение настроек между запусками. Overlay ограничен
