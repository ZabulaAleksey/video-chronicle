...
## 2026-08-11 — перенос проекта в `~/projects`

### Задача

Перенести приложение, локальные FFmpeg-зависимости и Git-историю из общей директории в `~/projects/video-chronicle`, устранить конфликты и подготовить отдельную публикацию.
...
Код, `.gitignore`, Git-история и оба FFmpeg-каталога перенесены в `~/projects/video-chronicle`. Старый remote сохранён как `legacy`, новый GitHub-репозиторий назначен `origin`.
...
3. Переместить проект в `~/projects/<project>`.
