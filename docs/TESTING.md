...
| Main/timeline/date/advanced navigation, reorder, meaningful button states и palette-independent surface | GUI/component/render | `tests/test_gui_application.py` | `GUI-NAV-AC-001–004`, `EDIT-AC-001/004` |
| Единый date/time formatter, explicit QuickTime wall-clock priority, typography, font fallback, независимый preview и multi-item export | unit/GUI/integration | `tests/test_metadata_date_engine.py`, `tests/test_overlay.py`, `tests/test_gui_application.py`, `tests/test_nondestructive_editing.py` | `DATE-AC-004`, `OVERLAY-AC-001–006`, `AC-004/010` |
| Настройки без повторного inspection, source freshness и delta-analysis новых/изменённых файлов | unit/GUI/component | `tests/test_core_extraction.py`, `tests/test_gui_application.py`, `tests/test_nondestructive_editing.py` | `GUI-APP-AC-004`, `EDIT-AC-001/004` |
| Join/Chronicle invariant, явный GUI-label «Объединение — без даты и времени», CLI parity, GUI round-trip и оба real exports | matrix/GUI/integration | `tests/test_modes.py`, `tests/test_gui_application.py`, `tests/test_ffmpeg_smoke.py` | `MODE-AC-001–003`, `AC-003/004/008` |
