...
- **GUI-TOOLS-004 — Приоритет основной задачи.** По умолчанию открыта вкладка
  «Дата и время», а кодек, CRF, preset, пути инструментов и cache находятся во
...
- **GUI-TOOLS-AC-004.** Component test подтверждает default-вкладку «Дата и
  время», вторичную вкладку «Дополнительно» и видимое объяснение cache.
...
- Выбор durable хранилища проекта и библиотек валидации после MODEL-001.
  SQLite, Pydantic и ExifTool остаются кандидатами, а не утверждёнными
  компонентами. OTIO принят только как removable optional adapter по
  INTEROP-001 и не входит в core/project schema. PySide6 выбран для
  GUI-оболочки GUI-001.
